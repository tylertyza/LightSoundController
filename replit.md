...
- **Real-time Data Streaming and State Restoration Fixes** (July 18, 2025): Fixed WebSocket real-time updates by adding proper device update message handling to eliminate reliance on polling-only updates. Corrected device state restoration timing to capture state when effects start rather than when they end, ensuring proper looping effect behavior. Enhanced scene gradient background functionality by improving color data extraction and ensuring new scenes include proper color arrays for gradient generation. Added manual "Refresh Status" button in Device Lab for immediate device status updates
- **Mobile UX Improvements** (July 18, 2025): Disabled scrolling of Creative Studio when mobile panels are open to prevent accidental scrolling. Completely removed debouncing from lighting controls for immediate response. Fixed mobile lighting effects to properly deselect after single tap by preventing double-trigger events and adding proper focus management. Mobile panels now only close via button or menu switching with no backdrop dismissal, and added bottom padding to prevent mobile menu from hiding controls
- **Fixed Device State Restoration and Performance** (July 18, 2025): Enhanced device state restoration by fetching fresh device information before saving state, ensuring proper restoration when effects are cancelled. Improved color and brightness control performance by optimizing debouncing mechanism (reverted to 150ms for stability). Fixed async handling in route handlers for triggerEffect calls to prevent timing issues. Removed lighting effects from Light Studio panel as requested
- **Mobile Bottom Navigation** (July 18, 2025): Added fixed bottom navigation pill for mobile with three creatively named buttons: Device Lab (device management), Creative Studio (main soundboard), and Light Studio (lighting controls). Fixed missing settings icon and used more intuitive icons. Desktop keeps arrow icons in header. Mobile navigation has blue active state indicators, proper touch targets, and adequate spacing to prevent content overlap
- **Device State Restoration** (July 18, 2025): Fixed lighting effects to properly restore device states when cancelled or finished. Added device state saving before effects and restoration after completion. Effects now return lights to their original color, brightness, and power state when stopped
- **Removed Unwanted Preset Effects** (July 18, 2025): Removed the preset lighting effects (Breathe, Pulse, Strobe) that reappeared in the Lighting Effects section. The section now only shows the actual lighting effects from the database, restoring the proper database-driven effects display
- **Device Status Updates and Performance** (July 18, 2025): Fixed colored device status circles to update every 2 seconds by adding periodic state polling to LIFX UDP service. Added debouncing to lighting controls for smooth multi-device editing without lag. Color, brightness, and temperature changes now batch updates with 150ms delay for optimal performance
- **Mobile Interface Bug Fixes** (July 18, 2025): Fixed scrolling inside Light Studio on mobile with lots of devices by creating proper scrollable container with touch-pan-y support. Fixed clicking on scenes and lighting effects on mobile by removing preventDefault from touch events to allow proper mobile interaction
- **Mobile Swipe-to-Dismiss Notifications** (July 18, 2025): Enhanced toast notifications with swipe-to-dismiss functionality on mobile devices, allowing users to swipe up to dismiss notifications with improved touch handling
- **LIFX Soundboard Branding** (July 18, 2025): Updated main application title and heading to "LIFX Soundboard" for consistent branding throughout the app
- **Creative Naming Convention** (July 18, 2025): Updated all panel titles to use consistent creative naming - Device Lab (device management), Creative Studio (main soundboard), and Light Studio (lighting controls). Updated modal sliders to match lighting controls styling with blue progress indicators for better visual consistency. Added collapsible functionality to adopted devices sections in both Light Studio panel and add effect modal for better space management
